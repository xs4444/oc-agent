...
local log = io.open("/home/dirt3.out", "w")
...
local found = {}
for slot = 1, total do
  local ok, s = pcall(inv.getStackInInternalSlot, inv, slot)
  if ok and s then
...
local ok, sel = pcall(robot.getSelectedSlot)
if ok then
  local selItem = inv.getStackInInternalSlot(sel)
  print("SELECTED_SLOT: " .. sel .. " = " .. (selItem and (selItem.name .. "x" .. selItem.size) or "empty"))
else
  print("SELECTED_SLOT_ERR: " .. tostring(sel))
end
-- 检测机器人脚下/面前的方块
local ok2, down = pcall(robot.detectDown)
print("DETECT_DOWN: " .. tostring(ok2 and down or "err"))
local ok3, up = pcall(robot.detectUp)
print("DETECT_UP: " .. tostring(ok3 and up or "err"))
]]
local e = serialization.serialize(probe):gsub("\\", "\\\\"):gsub("|", "\\|")
local rw = cmd("write|/home/probe_dirt3.lua|" .. e, 20)
lg("WRITE: " .. tostring(rw))
local r1 = cmd("exec|lua /home/probe_dirt3.lua", 30)
