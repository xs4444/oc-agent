 ...
 **side** - See the [Sides API](../api/sides.md) for a list of possible sides.

### TankWorldControl Trait API

- `compareFluid(side:number):boolean`
  Compares fluid in the selected tank (requires a [Tank upgrade](../item/tank_upgrade.md)) to fluid in the world or in an external tank on the specified side of the robot.
...
- `fill(side:number[, count:number]):boolean`
  Fills the specified amount of fluid from the selected tank (requires a [Tank upgrade](../item/tank_upgrade.md)) into the world or a tank in front of the robot. If no amount is specified, the robot will try to fill the target tank with 1000mB of fluid. If there is not enough fluid to fill a block, or not enough space in the target tank, the operation will fail with no fluids lost.
...
**side** - if given the robot will try to 'right-click' only on the surface as specified by side, otherwise the robot will try all possible sides. See the [Sides API](../api/sides.md) for a list of possible sides.
...
- `selectTank(tank:number)`
  Selects the specified tank (if robot contains more than one [Tank upgrade](../item/tank_upgrade.md)). Any tank operations will use this tank.
...
- `compareFluidTo(tank:number):boolean`
  Tests whether the fluid in the selected tank (in the robot inventory) is the same as in the specified tank (requires [Tank upgrade](../item/tank_upgrade.md)).
...
- `move(direction:number):boolean`
  Move in the specified direction. Valid movement sides are: front, back, top, bottom (see [Sides](../api/sides.md)).
