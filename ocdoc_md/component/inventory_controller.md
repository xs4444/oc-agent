...
This component is provided by the [inventory controller upgrade](../item/inventory_controller_upgrade.md).
...
Note that the robot cannot drop items into it's own inventory, attempting to do so will cause this to throw an error. You need to use `robot.transferTo` from the [Robot API](../api/robot.md) to do so.
...
Note that the robot cannot suck items from it's own inventory, attempting to do so will cause this to throw an error. You need to use `robot.transferTo` from the [Robot API](../api/robot.md) to do so.
- `equip():boolean`
...
Print meta-info about the item in the position indicated by [slot](http://ocdoc.cil.li/api:robot)
