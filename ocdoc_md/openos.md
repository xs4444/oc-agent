...
The purpose of this page of documentation is to focus on the installation and command line interface of OpenOS. This page begins with the assumption you have turned on a computer (of any tier) with the [OpenOS floppy disk](item/openos_floppy.md) and an empty [hard drive](item/hard_disk_drive.md) (of any tier) in the computer.
...
![](media/first_boot.png)

The computer at this point is usable and, assuming you have a [hard drive](item/hard_disk_drive.md) in the [case](block/computer_case.md) of the computer, you have writable storage space to create files. However you've only booted the system using the [OpenOS floppy disk](item/openos_floppy.md). In other words, your current directory is read-only; i.e. you cannot create or modify files in your current directory.

To list what filesystems are available, run `mount`

![](media/api/first_mount.png)
...
- label
  ...
  The last column displays the filesystem's label or the filesystem's component address in the case it has no label. [Loot disks](item/loot_disks.md) always have their loot disk name as the filesystem label. You may notice that the filesystem mounted on `/mnt/0e7/` in my example has a very long "label". This is because I am using a brand new tier 1 hard drive which doesn't have a label yet and thus its address is displayed which causes the terminal to wrap the text.
...
![](media/api/reboot_ready.png)

Running `mount` again will show that rootfs is now (rw).

![](media/api/system_ready.png)
