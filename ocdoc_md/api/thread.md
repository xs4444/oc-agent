...
1. The [thread](thread.md#thread-api) api, or the static functions, provided by `require("thread")`
2. The [thread handle](thread.md#thread-handle-api) api, or the api available the thread _objects_ created by `thread.create`. In this documentation these thread handles will be denoted by just `t`.

## Thread API

- `thread.create(thread_proc: function[, ...]): table`

  Starts a new thread executing the function `thread_proc` and returns its thread handle, see [Thread Handle API](thread.md#thread-handle-api). This method takes an optional `...` which is passed to `thread_proc`. The runtime of the thread continues autonomously.
...
Assuming the user presses ^c to send an interrupt
Output
```
main program
input: ^c
