# Event API
The Event API provides a basic event system to allow your code to react to [signals](../component/signals.md) sent by the OS or other programs/libraries.
...
- `event.pullMultiple(...): ...` (Since 1.5.9)
...
This is only an alias to [computer.pushSignal](computer.md). This does not modify the arguments in any way. It seemed logical to add the alias to the event library because there is also an `event.pull` for signals.
