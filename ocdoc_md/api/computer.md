...
- `computer.address(): string`
  The [component address](../component/component_access.md) of this computer.
...
- `computer.runlevel(): string|number`
  ...
  Returns the current [runlevel](https://en.wikipedia.org/wiki/Runlevel) the computer is in. Current Runlevels in OpenOS are:
...
- `computer.users(): string, ...`
  ...
  Please see [the user rights documentation](../computer_users.md).
...
- `computer.pullSignal([timeout: number]): name, ...`
  ...
  Generally it is more convenient to use `event.pull` from the [event](event.md) library. The return value is the very same, but the `event` library provides some more options.
