...
The rc system automates running scripts as services and is generally used for starting scripts when the system is booting up. It is one tool of many provided by OpenOS for automatic script execution, as documented [here](../tutorial/autorun_options.md).

OpenOS rc is inspired by [OpenRC](https://wiki.gentoo.org/wiki/OpenRC). "rc" stands for "run command".

There are two components of the rc system; [rc scripts](rc.md#rc-scripts) and [the rc controller](rc.md#the-rc-controller).
...
RC scripts are lua scripts that require [minimal setup](rc.md#minimal-setup), their [method](rc.md#rc-script-methods) api is flexible, and can also benefit from an [rc configuration](rc.md#rc-script-configuration).
...
A proper rc script needs only to define a `start` method. The design of the rc system anticipates that rc scripts can be started, and through various workflows with the [controller](rc.md#the-rc-controller), the system levarages the `start` method.
