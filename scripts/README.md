...
| `build_all.py` | **一键构建+测试**：build_single + make_manifest + 产物回归（117 项）。发版前必跑（否则版本不 bump，服务器嗅探不到——v0.3.2 教训） | `python scripts/build_all.py` |
| `release_check.py` | **发版安全检查**：版本 bump、清单完整性、字节匹配、脚本语法、git 干净度。全 PASS 才能打 tag | `python scripts/release_check.py` |
