---
name: ocvm-testing
description: ocvm 模拟器测试（本机 ~/oc-test/ocvm，192.168.31.7）。Triggers on "ocvm", "ocvm 测试", "modular 测试", "模拟器测试", "VM 测试", "remote_pit", "remote_battery"。涵盖 VM 启动/部署/远控 E2E 全流程、test_harness 远程套件（39 用例+10 场景电池）、legacy SSH 驱动 tools/ocvm_test.py 的 EXTRA_FILES 语法与 modular 测试。
---

# ocvm 模拟器测试（本机）

> ocvm 是 OpenComputers 的 C++ 模拟器。**本机（192.168.31.7，用户 hcj）就是测试服务器**
> ——旧文档里的"192.168.31.75 远程 + 密码 hcj2005 + `cd "F:\mie agent"`"全部过时作废。
...
```bash
export OCVM_HOST=192.168.31.7 OCVM_USER=hcj OCVM_PASS=<本机密码或留参>
...
EXTRA_FILES="src/agent=agent,src/agent/init.lua=agent/agent.lua" \
  OCVM_HOST=192.168.31.7 OCVM_USER=hcj OCVM_PASS=<...> \
...
- `tools/ssh_ubuntu.py` 默认 IP 仍是 **192.168.31.75（过时**，真机 192.168.31.7）——
  用 `UBUNTU_HOST=192.168.31.7 python3 tools/ssh_ubuntu.py ...` 覆盖
