...
| 代码量 | ~9300 行（模块化 8 文件 `src/agent/`，构建单文件 agent.lua 490KB） | 库 + 示例（~数千行） | ~1797 行 agent-loop 核心 + 大量包 |

## 核心循环

| 维度 | agent.lua | oc-ai | pi |
|------|-----------|-------|-----|
| 主循环 | TUI（`/help`、`/model`、`/resume` 等命令 + 增量渲染 + 会话管理）+ 内部两阶段循环（LLM 调用 → 工具执行 → 再调 LLM） | `generateText` 内建工具循环（`maxSteps` 上限），顶层由用户脚本驱动 | `agentLoop` 独立运行时（`agent-loop.ts`），事件流驱动，支持并行工具批处理 |
...
| 消息历史 | 持久化到 `/home/sessions/`（JSONL + .txt 双格式），`/new` 归档 + `/resume` 恢复 | 调用方自行管理 | 状态管理在 agent runtime 内 |

## 工具系统

| 维度 | agent.lua | oc-ai | pi |
|------|-----------|-------|-----|
| 工具数量 | 11（read_file+行切片/write_file/edit_file/append_file/list_files/glob_files/grep_files/json_query/web_search/shell_execute/**subagent_call**） | oc-code：7（read_file/write_file/edit_file/list_directory/glob/grep/shell） | 大量（文件/进程/网络/权限/插件…） |
...
|------|----------------|-------|-----|
| 内存（T3.5 = 1MB） | 三重裁剪：历史条数（MAX_HISTORY=100，内存自适应缩放）+ 字节（MAX_HISTORY_BYTES 随 mem 缩放）+ 工具结果截断（3KB）；compaction 压缩大对话 + 压缩后 resume 重渲染 | 文档提醒"limited memory, avoid large buffers"，无主动裁剪 | 不适用（宿主机内存充足） |
...
| oc-ai | `edit_file` 工具（行级编辑，替代整文件 write_file，省 token/内存）——**已落地**（唯一性检查 + 20KB 上限） | 高（已完成） |
| oc-ai | OPPM 打包分发（`oppm register`） | 低（手动安装够用） |
| pi | 并行工具批处理（`executeToolCallsParallel`，当前工具相互独立时提速） | 中（OC 单线程，收益有限，需谨慎） |
| pi | 事件流驱动架构（为 TUI/日志/断点重续做铺垫） | 低（当前 TUI 增量渲染够用） |
| pi | 工具调用失败恢复（truncated message → 重发或降级） | 中（目前失败即报错回传） |

## 结论

三个项目定位互补：**oc-ai 是 OC 生态内最成熟的 AI 库**（流式/结构化输出/声明式工具），**pi 是宿主机构建通用 agent 的参考架构**（并行工具/事件流/多 provider），**agent.lua 是唯一专为 OC 资源约束设计的单文件 agent**。对比后 agent.lua 已落地：移除 `execute_lua`（任意代码执行）+ 移除 `calc`/`text_ops`/`component_*` 工具（精简为 11 工具集）、`edit_file`（借鉴 oc-ai，唯一性检查 + 20KB 上限）、`append_file`（pi 没有的差异化：流式追加内存恒定）、`read_file` 行切片（借鉴 pi 的 offset/limit，负 offset tail）、`glob_files`/`grep_files`（借鉴 oc-code）、HTTP 自动重试（1h 预算）、compaction（借鉴 opencode anchored summary 增量更新 + 压缩后 resume 重渲染）、会话归档 + `/resume` 恢复（TUI 重建历史缓冲）、append-only 会话日志（消内部 O(n²) 整写）。
