...
35. **v6 read/write 必须二进制模式 — 文本模式会静默损坏所有非 ASCII 文件**
    （真机实证，2026-09-15）：

    **现象**：把一个 31500B、含中文注释的文件用 v6 `read` 读回，只得到 **15962B**；
    在机器人本机用 `io.open("rb")` 自校验却是完整 `len=31500`（文件本身没坏）。
    更隐蔽的一种：部署 `remote_host.lua` 后 size 完全正确（16989 字节）、
    头尾字节也一致，但中间 `─`（U+2500, `e2 94 80`）被替换成 U+FFFD
    （`ef bf bd`）—— **等长替换**，所以任何 size/sum 之外的校验都可能漏掉。

    **根因（源码级）**：OpenOS 的 `f:read(n)` 里 n 的单位由 `mode.b` 决定
    （`lib/core/full_buffer.lua:134-143` `readBytesOrChars`：`mode.b` → 
    `rawlen`/`string.sub` 按**字节**；否则 `unicode.len`/`unicode.sub` 按**字符**）。
    服务端原来 6 处 `io.open` 全用文本模式（`"r"`/`"w"`/`"a"`），于是：
    - `read`：`f:read(7000)` 读 7000 个**字符**。中文 3 字节/字 → 单块可达 ~21KB，
      **超出 modem 8192B 单包上限 → 静默截断**（不报错、不置 err）。
    - `write`/`move_file`：客户端按**字节**切块（`string.sub`），切点会劈开
      多字节字符；文本模式解码时半个字符 → U+FFFD。

    **崩溃边界**：v6 的 read/write 对**纯 ASCII** 一直是对的 —— 这也是为什么
    50KB 的 e2e（T8，内容是 `X`/`Z` 等 ASCII）能长期 11/11 全绿却掩盖了该缺陷。
    **凡是非 ASCII 内容（中文注释、emoji、GTNH 的本地化名）经 v6 传输即损坏。**

    **修复**：服务端全部 `io.open` 加 `b`（`"rb"`/`"wb"`/`"ab"`）；
    `remote_debug/remote_host.lua` 现为 6/6 二进制模式。
    **部署此类修复时的注意**：损坏的那条路径正是修复本身要走的路径，
    故须用**纯 ASCII 的 hex 摆渡**（hex 字符集是 ASCII，规避该缺陷），
    在机器人侧解码 + `loadfile` 编译校验 + 逐字节 bytesum 比对后再替换。
